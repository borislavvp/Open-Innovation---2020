...
class PharmacyMap extends React.Component<PharmacyMapProps, PharmacyMapState> {
	...
	calculateDirection(coordinates: GeolocationPosition['coords']) {
		...
		DirectionsService.route(
			{
				origin: new google.maps.LatLng(coordinates.latitude, coordinates.longitude),
				destination: new google.maps.LatLng(this.props.pharmacy.lat, this.props.pharmacy.lng),
	...
	showDirection = () => this.setState({ showDirection: !this.state.showDirection });
	onMarkerClick = () => {
		this.setState({
			...
			<div>
				...
				>
					{this.state.showingInfoWindow && (
						<InfoWindow anchor={this.state.activeMarker} onCloseClick={() => this.onInfoWindowClose()}>
							<div>
								<h3>{title}</h3>
								{this.renderRating(rating)}
								<p>
									<Translate value="pharmacies.distance" />
									<strong>
										{getDistance(loc.latitude, loc.longitude, lat, lng).toFixed(1)}
										km
									</strong>
								</p>
								{this.state.showDirection && (
									<IonButton expand="block" onClick={() => this.openGoogleMaps(lat, lng)}>
										GOOGLE MAPS
									</IonButton>
								)}
								<IonButton expand="block" onClick={() => this.showDirection()}>
									{this.state.showDirection ? (
										...
										<Translate value="pharmacies.navigate" />
									)}
								</IonButton>
							</div>
						</InfoWindow>
